local FindRoact = game:GetService("ReplicatedStorage"):FindFirstChild("Roact", true) --require(script.Parent.Parent.Roact)
local Roact
if FindRoact then
	if FindRoact:IsA("Folder") then
		local lib = assert(FindRoact:FindFirstChild("lib"), "Cannot find Library 'Roact'")
		Roact = require(lib)
	elseif FindRoact:IsA("ModuleScript") then
		require(FindRoact)
	else
		error("Could not find Roact.")
	end
end
